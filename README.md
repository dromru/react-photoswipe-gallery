...
| `uiElements` | Array | | Array of [custom UI elements](#custom-ui-elements) configs. Use it for [adding custom UI elements](https://photoswipe.com/adding-ui-elements/) ([example](https://github.com/dromru/react-photoswipe-gallery/blob/master/src/storybook/rotate-slide-button.stories.tsx)) |
| `onBeforeOpen` | Function | | Triggers before `PhotoSwipe.init()` call. Use it for something, that you need to do, before PhotoSwipe.init() call. It will receive PhotoSwipe instance as the first argument: `(photoswipe: PhotoSwipe) => void` |
